...
3. **Map the scenes & performance** — per scene/beat: action, camera, motion (animation principles), and — for talking scenes — dialogue tied to words. → `references/chassis.md`. **Sung job** (a finished song + lyrics arrived)? The Voice/Audio block and segmenting swap to → `references/delivery/singing.md`.
...
- **Only for sung jobs (a finished song + lyrics):** `references/delivery/singing.md` — the input contract, the transcript rule, the fidelity ladder (audio → black-screen video), the full-song segment workflow.
- **For ready-to-paste patterns:** `examples/pixar-disney.md` (spoken) · `examples/pixar-disney-singing.md` (sung).
