...
- **Kill the gaps — wall-to-wall vocals.** Every instrumental second is dead screen time the video edit must patch. Never open on `[Intro]`; omit `[Bridge]` / `[Instrumental]` / `[Break]` / `[Drop]` in a continuous song; no blank lines between lyric blocks; end on `[Outro]` with sung lines + `[End]`. Reinforce **positively** in Style ("vocal-forward, singer begins on the first beat, continuous vocals" — never "no X" phrases; negations go in Exclude Styles). Finish every keeper with an editor Crop pass. (→ `references/models/suno.md`.)
