...
- **Kill the gaps.** Never open on `[Intro]`; omit `[Bridge]` / `[Instrumental]` / `[Break]` / `[Drop]` in a continuous song; end on `[Outro: Cold End]` / `[End]`. Reinforce "continuous vocals, vocals start immediately" in Style. (→ `references/models/suno.md`.)
